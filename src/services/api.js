// VoltGuard - Frontend API Service Layer
...
export async function getAssets() {
  ...
  const stored = localStorage.getItem('voltguard_assets');
  if (stored) {
    try {
      return JSON.parse(stored);
    } catch (e) {}
  }
  localStorage.setItem('voltguard_assets', JSON.stringify(INITIAL_ASSETS));
...
export async function getMaintenance() {
  ...
  const stored = localStorage.getItem('voltguard_maintenance');
  if (stored) {
    try {
      return JSON.parse(stored);
    } catch (e) {}
  }
  localStorage.setItem('voltguard_maintenance', JSON.stringify(INITIAL_MAINTENANCE));
...
export async function createMaintenance(newTask) {
  ...
  localStorage.setItem('voltguard_maintenance', JSON.stringify(updated));
  return task;
}
